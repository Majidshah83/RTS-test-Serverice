...
class AnswerKeyController extends Controller
{
    public function answerKey(){
       ...
       return view('frontend.answer_key.index')->with('projects', $projects);

    }

    public function jobDetailAnswer($id='') {
        if ($id) {
            $project  = Projects::where('ad_id', $id)->first();
            $postType = Job_Type::where('job_id',$id)->get();
            return view('frontend.answer_key.post')
                ->with('postTypes', $postType)
