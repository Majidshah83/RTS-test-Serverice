@extends('admin.app')
 @section('title', 'Admin|Current Center')
...
@section('content')
              ...
              <div class="row breadcrumbs-top d-inline-block">
                <div class="breadcrumb-wrapper col-12">
                  <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Current Center
                    </li>
                  </ol>
                </div>
              </div>
             ...
             <div class="row">
              <div class="col-12">
                ...
                <div class="card-header">
                  <h4 class="form-section" align="center"></i>Current Center</h4>
                  <a class="heading-elements-toggle"><i class="la la-ellipsis-v font-medium-3"></i></a>
                  <a href="{{url('/add/center')}}">
                    <button type="button" class="btn btn-success btn-min-width box-shadow-2 mr-1 mb-1"><i class="ft-plus"></i>Add New Center</button>
                  </a>
                </div>
                    ...
                    <table class="table table-striped table-bordered zero-configuration">
                      <thead>
                        <tr>
                          ...
                          <th width="20%">City</th>
                          <th width="20%">Candidate Capacity</th>
                          <th width="30%"><center>Action</center></th>
                        </tr>
                        </thead>
                       <tbody>
                       @foreach($center as $key=>$allcenter)
                          <tr>

                            <td>{{++$key}}</td>
                            <td>{{$allcenter->center_name}}</td>
                            <td>{{$allcenter->city->city_name}}</td>
                            <td>{{number_format($allcenter->candidate_per_center)}}</td>
                            <td>
                            <center>
                            <a href="{{url('/edit/center', $allcenter->center_id)}}">
                            <button type="button" class="btn btn-outline-success" aria-haspopup="true" aria-expanded="false">Edit</button>
                            </a>
                            <button type="button" onclick="remove({{$allcenter->center_id}})" data-toggle="modal"  class="btn btn-outline-danger delete_class" data-tooltip="tooltip" title="Delete">Delete</button>
                            </center>
                            </td>
  ...
  <script type="text/javascript">
    function remove(id) {
        swal({
          title: "Are you sure?",
          text: "You are about to delete the center, this procedure is irreversible!",
          icon: "warning",
          buttons: {
                  cancel: {
                      text: "No, cancel plx!",
      ...
      .then((isConfirm) =>{
          if (isConfirm) {
              $.ajax({
                type:"get",
                url:"{{ url('/center/delete') }}",
                data:{id:id},
                success: function(response){
                swal("Deleted!", "Your province has been center.", "success");
                setTimeout(function() {
                   location.reload();
                }, 1000);
               }
            ...
            swal("Cancelled", "Your center is safe", "error");
        }  
      });
    }
